...
var WhiteSpace = tokens[0], LineComment = tokens[1], LCurly = tokens[2], RCurly = tokens[3], LSquare = tokens[4], RSquare = tokens[5], Pipe = tokens[6], Newline = tokens[7], Ellipsis = tokens[8], Arrow = tokens[9], NumberLiteral = tokens[10], TimeSpan = tokens[11], LengthFunction = tokens[12], After = tokens[13], OnEvent = tokens[14], If = tokens[15], Else = tokens[16], When = tokens[17], Label = tokens[18], Checkpoint = tokens[19], Directive = tokens[20], Assignment = tokens[21], StateNodeName = tokens[22];
var Parser = /** @class */ (function (_super) {
    ...
    function Parser() {
        ...
        $.RULE("condition", function () {
            $.OR([
                { ALT: function () { $.CONSUME(StateNodeName); } },
            ]);
        });
        // $.RULE("guard", () => {
        //   $.OR([
