...
var WhiteSpace = tokens[0], LineComment = tokens[1], LCurly = tokens[2], RCurly = tokens[3], LSquare = tokens[4], RSquare = tokens[5], Pipe = tokens[6], Newline = tokens[7], Ellipsis = tokens[8], Arrow = tokens[9], NumberLiteral = tokens[10], TimeSpan = tokens[11], LengthFunction = tokens[12], After = tokens[13], OnEvent = tokens[14], IfCondition = tokens[15], When = tokens[16], Label = tokens[17], Checkpoint = tokens[18], Directive = tokens[19], Assignment = tokens[20], StateNodeName = tokens[21];
var Parser = /** @class */ (function (_super) {
    ...
    function Parser() {
        ...
        $.RULE("guard", function () {
            $.OR([
                { ALT: function () { $.CONSUME(IfCondition); } },
                {
                    ALT: function () {
                        $.CONSUME2(When);
                        $.OR3([
