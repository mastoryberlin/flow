...
var visitor;
function useFlowToStatechart(flow, id, variant, validSenders) {
...
function stateNodeToJsonRecursive(fqPath, variant, node, parentInfo) {
    ...
    if (node) {
        var json = {};
        var autoProceedFromLastChild = undefined;
        if (children === null || children === void 0 ? void 0 : children.length) {
            if (node.parallel) {
                json.type = 'parallel';
            }
                ...
                for (var _i = 0, _d = children.filter(function (c) { return c.name !== '*'; }); _i < _d.length; _i++) {
                    var k = _d[_i];
        ...
        var testNodeName = node.name;
        // ========================================================================================================================
        // Interactive Conversations
        // ========================================================================================================================
        if (util_1.promptStateRegExp.test(testNodeName)) {
            var nluContext_1 = parentInfo === null || parentInfo === void 0 ? void 0 : parentInfo.nluContext;
            if (!nluContext_1) {
                console.error("Cannot obtain data for enterInteractive and leaveInteractive invocations: parentInfo.nluContext is undefined (path: ".concat(fqPath, ")"));
            }
            else {
                // ================================================================
                ...
                json.entry = {
                    ...
                    params: __assign({ pathInFlow: fqPath.split('.').slice(0, -1), contextId: '907415bb-cea1-4908-aa7c-548a27da14f2' }, nluContext_1)
                };
                ...
                json.on = {
                    INTENT: __spreadArray([], nluContext_1.intents.map(function (intentName) { return ({
                        ...
                        guard: { type: 'isIntentName', params: { intentName: intentName } }
                    }); }), true)
                };
        ...
        var _e = interpretTransitions(fqPath, node), on = _e.on, after = _e.after, always = _e.always;
        // ========================================================================================================================
        ...
        if (directive) {
            ...
            switch (directive.name) {
                ...
                default:
                    var valid = true;
                    for (var _f = 0, _g = Object.entries(directives_1.supportedDirectives); _f < _g.length; _f++) {
                        var _h = _g[_f], dname = _h[0], d = _h[1];
                        if (directive.name === dname) {
                            var args_3 = d.args(directive.arg);
                            for (var _j = 0, _k = ['entry', 'exit', 'invoke']; _j < _k.length; _j++) {
                                var key = _k[_j];
                                if (key in d) {
                                    var impl = d[key];
                                    if (Array.isArray(impl)) {
                                        json[key] = [];
                                        for (var _l = 0, impl_1 = impl; _l < impl_1.length; _l++) {
                                            var i = impl_1[_l];
                                            var out = {};
                                            for (var _m = 0, _o = Object.entries(i); _m < _o.length; _m++) {
                                        ...
                                        var out = {};
                                        for (var _q = 0, _r = Object.entries(impl); _q < _r.length; _q++) {
                                            var _s = _r[_q], k = _s[0], v = _s[1];
                                            out[k] = typeof v === 'function' ? v(args_3) : v;
                                        }
                                        if (key === 'invoke') {
                                            if ('src' in out) {
                            ...
                            if ('always' in d) {
                                ...
                                if (typeof def === 'function') {
                                    always = def(args_3, rootId);
                                }
                                else {
                                    var cond = {};
                                    for (var _t = 0, _u = Object.entries(def.cond); _t < _u.length; _t++) {
                                        var _v = _u[_t], k = _v[0], v = _v[1];
            ...
            if (invoke.src) {
                ...
                json.states = {
                    __DIRECTIVE_ACTIVE__: { on: { SUBFLOW_DONE: '__DIRECTIVE_DONE__' }, invoke: invoke },
        ...
        if (node.message && node.message.sender) {
            var _w = node.message, kind = _w.type, sender = _w.sender;
            // @ts-ignore
            ...
            json.states = __assign({ __SEND_MESSAGE_ACTIVE__: {
                    ...
                    always: node.final ? "#".concat(rootId, ".__FLOW_DONE__") : json.initial ? __spreadArray([json.initial], always, true) : __spreadArray([], always, true),
                    after: node.final ? {} : __assign({}, after)
                } }, json.states);
        ...
        // ========================================================================================================================
        if (node.checkpoint) {
            var childStates_1 = json.states;
            if (autoProceedFromLastChild) {
                childStates_1[autoProceedFromLastChild].after = json.after;
            }
            json.states = __assign({ __CHECKPOINT_ACTIVE__: {
                    entry: [
                        {
                            type: 'offerExit',
                            params: __assign({ path: fqPath }, node.checkpoint)
                        }
                    ],
                    on: {
                    ...
                    always: node.final ? "#".concat(rootId, ".__FLOW_DONE__") : __spreadArray([], always, true),
                    after: node.final ? {} : __assign({}, after)
                }, __CHECKPOINT_REENTER__: {
                    always: (_c = json.initial) !== null && _c !== void 0 ? _c : '__CHECKPOINT_DONE__'
                } }, childStates_1);
            json.always = [];
            always = [];
            json.after = {};
        ...
        var _x = interpretTransitions(rootId), on = _x.on, after = _x.after, always = _x.always;
